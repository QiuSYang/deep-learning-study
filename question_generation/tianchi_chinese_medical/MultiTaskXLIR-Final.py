...
class XLDecoder(torch.nn.Module):
    ...
    def forward(self, input_ids, encoder_rep, input_mask, decode_input, decode_target, use_beam_search, beam_width):
        ...
        if decode_input is not None:  # 代表训练模式
            ...
            input_logits = torch.einsum("bid,bjd->bij", self.copy_output(core_out),
            ...
            if not use_beam_search:  # 使用贪心搜索 ==> 验证集
                dec_list = []
                decode_ids = torch.full(size=(bsz, 1), fill_value=args["start_token_id"], dtype=torch.int32).long().to(
                    device)
                ...
                for i in range(1, self.seq_len + 1):
                    ...
                    vocab_logits = torch.nn.functional.linear(input=output, weight=self.word_emb.weight)
                    vocab_prob = torch.softmax(vocab_logits, dim=-1)
                    input_logits = torch.einsum("bd,bjd->bj", self.copy_output(core_out),
                    ...
                    vocab_prob = vocab_prob * mode_sig
                    vocab_prob = torch.scatter_add(vocab_prob, dim=1, index=input_ids,
                    ...
                    for j in range(bsz):
                        ...
                        for k in range(beam_width):
                            ...
                            if args["end_token_id"] in dec_topK_sequences[ind]:
                                candidate_list.append({
                                    "add_logit": 0, "add_seq_len": 0, "affiliate_k": k,
                                    "add_token_id": args["end_token_id"],
                                    "sort_logits": dec_topK_log_probs[ind] / (
                                                dec_topK_seq_lens[ind] ** args["beam_length_penalty"])
                                })
                            else:
                                ...
                                for l in range(beam_width):
                                    aff = l if i == 1 else k
                                    candidate_list.append({
                                        "add_logit": k_logps[l], "add_seq_len": 1, "affiliate_k": aff,
                                        "add_token_id": k_indices[l],
                                        "sort_logits": (dec_topK_log_probs[ind] + k_logps[l]) / (
                                                    (dec_topK_seq_lens[ind] + 1) ** args["beam_length_penalty"])
                                    })
                        ...
                        for k in range(beam_width):
                            ...
                            r_ind = bsz * k + j
                            father_seq, father_logits, father_len = c_dec_topK_sequences[ind], c_dec_topK_log_probs[
...
class Main(object):
    ...
    def valid(self):
        ...
        rouge_l = []
        with torch.no_grad():
            for item in tqdm(self.valid_loader):
    ...
    def test(self):
        ...
        with open("submit.json", "w", encoding="UTF-8") as fw:
...
if __name__ == "__main__":
    ...
    args = {
        ...
        "lr_decay_steps": 150,  # 15000,
        "max_steps": 500,  # 16000,
        "min_lr_rate": 1e-9,
        "eval_interval": 1000,
        "save_interval": 100,  # 1000,
        "save_path": "ModelStorage/cloudwalk_qg_enhance.path",  # "ModelStorage/final_g.pth",
        "load_path": "ModelStorage/final.pth",  # "ModelStorage/xl_dureader_drmc.pth",
    ...
    dataset_path = "DataSet/cloudwalk_dataset_enhancement.pkl"  # "DataSet/multi_task.pkl"
    ...
    # tokenizer = BertTokenizer(vocab_file="PATH/vocab.txt")
    tokenizer = BertTokenizer.from_pretrained("hfl/chinese-roberta-wwm-ext-large")
    if args["vocab_size"] != len(tokenizer):
        args["vocab_size"] = len(tokenizer)

    if sys.argv[1] == "train":
        torch.distributed.init_process_group(backend="nccl", rank=0, world_size=1, init_method='tcp://localhost:7003')
        args["is_train"] = True
        ...
        m = Main(None, None, test_flag=True, test_items=x["train_items"])
        m.test()
