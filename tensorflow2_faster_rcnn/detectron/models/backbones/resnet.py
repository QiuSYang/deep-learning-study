...
class _Bottleneck(tf.keras.Model):
    def __init__(self, filters, block,
        ...
        self.stride = stride
        self.out_channel = filter3

        self.conv2a = layers.Conv2D(filter1, (1, 1), strides=(self.stride, self.stride),
                                    kernel_initializer='he_normal',
                                    name=conv_name_base + '2a')
        self.bn2a = layers.BatchNormalization(name=bn_name_base + '2a')

        self.conv2b = layers.Conv2D(filter2, (3, 3), padding='same',
                                    kernel_initializer='he_normal',
        ...
        if self.down_sampling:
            self.conv_shortcut = layers.Conv2D(filter3, (1, 1), strides=(self.stride, self.stride),
                                               kernel_initializer='he_normal',
