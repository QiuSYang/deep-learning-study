...
class BBoxHead(tf.keras.Model):
    def __init__(self, num_classes,
        ...
        self.rcnn_class_conv2 = layers.Conv2D(1024, (1, 1), name='rcnn_class_conv2')
    ...
    def loss(self,
        ...
        rcnn_class_loss = self.rcnn_class_loss(rcnn_labels=rcnn_labels,
                                               rcnn_class_logits=rcnn_class_logits,
                                               rcnn_label_weights=rcnn_label_weights)
        rcnn_bbox_loss = self.rcnn_bbox_loss(rcnn_delta_targets=rcnn_delta_targets,
                                             rcnn_deltas=rcnn_deltas,
                                             rcnn_delta_weights=rcnn_delta_weights)
    ...
    def get_bbox(self, rcnn_probs, rcnn_deltas, rois, img_metas):
        ...
        detections_list = [
            self._get_bboxes_single(rcnn_probs[i], rcnn_deltas[i],
                                    rois[i], pad_shapes[i])
    ...
    def _get_bboxes_single(self, rcnn_probs, rcnn_deltas, rois, img_shape):
        ...
        # 归一化坐标逆转为标准坐标，与图像尺寸保持一致
        refined_rois *= tf.constant([H, W, H, W], dtype=tf.float32)
        window = tf.constant([0.0, 0.0, H*1.0, W*1.0], dtype=tf.float32)
